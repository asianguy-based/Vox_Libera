  ...
  return (
    <div>
      <div className="flex items-center justify-center gap-4 mb-4">
         <h1 className="text-2xl font-bold text-current opacity-90">{labels?.selectCategory || "Select a Category"}</h1>
         <button 
           onClick={() => setIsReordering(!isReordering)}
           className={`px-3 py-1 text-xs sm:text-sm font-semibold rounded-full border transition-all ${isReordering ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-transparent text-slate-500 border-slate-300 hover:border-blue-400 hover:text-blue-600'}`}
         >
